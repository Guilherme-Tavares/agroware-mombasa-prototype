...
export default function FeedStockList() {
  ...
  const stocks   = useFarmStore((s) => s.feedStocks)
  ...
  const feedName = (id: string) => feeds.find((f) => f.id === id)?.name ?? 'Alimento'
  ...
  const filtered = useMemo(() => {
    ...
    return stocks
      ...
      .filter((s) => !q || feedName(s.feedId).toLowerCase().includes(q))
      .sort((a, b) => feedName(a.feedId).localeCompare(feedName(b.feedId)))
  ...
  return (
    <ConsultScreen
      ...
      search={search}
      onSearch={setSearch}
      searchPlaceholder="Buscar por alimento..."
      showInactive={showInactive}
      onToggleInactive={() => setShowInactive((v) => !v)}
      onNew={can.writeHusbandry ? () => navigate('/feed-stock/new') : undefined}
      newLabel="Entrada"
    >
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description={search ? 'Ajuste a busca.' : 'Registre entradas de estoque de alimentos.'}
          action={can.writeHusbandry ? { label: 'Registrar entrada', onClick: () => navigate('/feed-stock/new') } : undefined}
        />
      ) : (
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-body font-medium text-gray-900 truncate">{feedName(s.feedId)}</p>
